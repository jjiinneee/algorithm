...
public class Ex5 {
  public static void main(String[] args) {
    ...
    ArrayList<ArrayList<Node5>> graph = new ArrayList<ArrayList<Node5>>();
    ...
    for (int i = 0; i < E; i++) {
      ...
      graph.get(a).add(new Node5(b, cost));
    }
    ...
    for (int i = 0; i < V; i++) {
      ...
      for (int j = 0; j < graph.get(nodeIdx).size(); j++) {
        // 인접 노드를 선택한다.
        Node5 adjNode = graph.get(nodeIdx).get(j);
